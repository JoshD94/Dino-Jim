...
let test_player_skins () =
  ...
  let p1 = Player.create () in
  let p2 = Player.create () in
  Player.add_skin p2 SantaSkin.draw;
  Player.select_skin p1 SantaSkin.draw;
  Player.select_skin p2 SantaSkin.draw;
  let b =
    (Player.current_skin p1) 0. 0. = DefaultSkin.draw 0. 0.
    && (Player.current_skin p2) 0. 0. = SantaSkin.draw 0. 0.
